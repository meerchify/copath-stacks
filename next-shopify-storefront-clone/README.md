...
## 2b. Fill the homepage sections & menus (Shopify keys)

The storefront is laid out as a series of **slots** that each map to a specific
Shopify resource by its handle. While a slot is empty, the store renders a
labelled **placeholder showing the exact key** you need to create in Shopify
admin — so you always know what to add and where it will appear. As soon as the
matching collection/menu exists and has content, the real section renders in its
place.

**Homepage sections** — create a Shopify **collection** whose handle matches the
key, then add products to it:

| Section | Collection handle (key) |
| --- | --- |
| Featured | `hidden-homepage-featured-items` |
| Trending Now (carousel) | `hidden-homepage-carousel` |
| New Arrivals | `hidden-homepage-new-arrivals` |
| Bestsellers | `hidden-homepage-bestsellers` |
| Popular Collections (promo tiles) | any 2+ non-hidden storefront collections |
| Browse the Catalog | your whole catalog (automatic) |

> To set a collection's handle in Shopify: open the collection →
> **Search engine listing → Edit → URL handle**. The `hidden-` prefix keeps
> these collections out of the nav and search results so they only feed the
> homepage.

**Navigation menus** — create a **menu** (Online Store → Navigation) whose
handle matches the key:

| Slot | Menu handle (key) |
| --- | --- |
| Top category nav | `next-js-frontend-header-menu` |
| Footer "Collections" links | `next-js-frontend-footer-menu` |

**Product page** — the "You Might Also Like" carousel uses Shopify's automatic
product recommendations; it fills in once a product is related to others.
