    ...
    <>
      ...
      <Transition show={isOpen}>
        <Dialog onClose={closeCart} className="relative z-50">
          ...
          >
            <Dialog.Panel className="fixed bottom-0 right-0 top-0 flex h-full w-full flex-col border-l border-border bg-background/95 p-6 text-foreground backdrop-blur-xl md:w-[400px]">
...
function CheckoutForm() {
  ...
  const handleCheckout = () => {
    ...
    if (!checkoutUrl) {
      setMessage(
        "Checkout isn't available yet — connect your Shopify store to enable it.",
      );
      return;
    }
...
function CheckoutButton({ onCheckout }: { onCheckout: () => void }) {
  return (
    <button
      className="block w-full rounded-md bg-primary p-3 text-center text-sm font-semibold text-primary-foreground opacity-95 hover:opacity-100"
      type="button"
      onClick={onCheckout}
    >
      Proceed to Checkout
